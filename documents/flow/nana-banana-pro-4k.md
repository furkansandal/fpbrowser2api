# nana-banana-pro-4k  ·  Flow / nana-banana (görsel, 4k)

**Endpoint:** `POST /v1/videos`
**İç task tipi:** `veo_workflow`

Nana Banana Pro'nun **4K** varyantı. `nana-banana-pro` ile aynı Pro görsel üretim modeli olup tek farkı çıktının sabit **4K** çözünürlükte olmasıdır: sunucu `resolution`'ı her zaman `4k`'ya zorlar. Çıktı görseldir; sonuç `image_url` / `url`'den okunur.

## İstek Gövdesi

| Alan | Tip | Zorunlu | Default | Açıklama |
|------|-----|:------:|---------|----------|
| model | string | ✅ | — | `"nana-banana-pro-4k"` |
...
## Minimal örnek
```json
{ "model": "nana-banana-pro-4k", "prompt": "premium editorial product photo, luxury magazine style, soft studio light" }
```
...
```json
{
  ...
  "prompt": "create a premium 4k campaign image using all references",
  "aspect_ratio": "9:16",
  "images": [
...
## Notlar
- **resolution sabit `4k`'dır.** Sunucu `payload["resolution"] = "4k"` olarak override eder; istekteki değer yok sayılır.
...
- `nana-banana-pro` ile aynı: dahili `image_model_name="GEM_PIX_2"`, `n_frames=1`. Tek fark çözünürlüğün 4K'ya sabitlenmesidir. (`nana-banana-2-4k`'dan farkı: model adı `NARWHAL` yerine `GEM_PIX_2`.)
