# gpt-image2-1k  ·  GPT-Image2 (görsel)

**Endpoint:** `POST /v1/videos`
**İç task tipi:** `gpt_workflow`

ChatGPT (gpt-image-2) üzerinden **1K** çözünürlükte görsel üretir. Yalnızca `prompt` verilirse metin→görsel (text-to-image), bir veya birden fazla referans görsel verilirse görsel düzenleme (image edit) modunda çalışır. `duration` görsel modunda yalnızca `1` olabilir.

## İstek Gövdesi

| Alan | Tip | Zorunlu | Default | Açıklama |
|------|-----|:------:|---------|----------|
| model | string | ✅ | — | `"gpt-image2-1k"` (model adı 1K çözünürlüğü seçer) |
...
| size | string | ⬜ | (orandan türetilir) | Açık piksel boyutu (örn. `"1344x768"`). Bu modelde tier ile çelişen değer yok sayılıp orandan yeniden hesaplanır. |
...
| resolution | string | ⬜ | "1k" | Model adından gelir; gövdede verilse bile model adı (`gpt-image2-1k`) ile ezilir. |

> Zorunlu = ✅, Optional = ⬜

## Minimal örnek
```json
{ "model": "gpt-image2-1k", "prompt": "retro neon poster, cyberpunk girl" }
```
...
- **ratio / aspect_ratio:** İkisi eşdeğer; verilmezse `1:1`. Bu tier'da desteklenen oran→piksel tablosu:
  `1:1`=1024x1024, `3:2`=1216x832, `2:3`=832x1216, `4:3`=1152x864, `3:4`=864x1152, `5:4`=1120x896, `4:5`=896x1120, `16:9`=1344x768, `9:16`=768x1344, `21:9`=1536x640.
- **resolution / size_tier farkı:** Bu üç model çözünürlük dışında aynıdır. `gpt-image2-1k` → `resolution=1k`, `size_tier=1K`. Çözünürlük **model adıyla** seçilir; gövdedeki `resolution`/`size` alanları tier ile çelişiyorsa ezilir. 2K için `gpt-image2-2k`, 4K için `gpt-image2-4k` kullanın.
